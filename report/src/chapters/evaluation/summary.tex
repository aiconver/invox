...
\subsection*{Cross-Strategy Consistency (Format \& Style)}
\begin{figure}[H]
\centering
\begin{tikzpicture}
  \begin{axis}[
    ...
    % FPR_overall (schema formatting pass rate)
    ...
    % SC_macro (style consistency)
    \addplot[fill=blue!35, draw=blue!70!black] coordinates {
      (S1.1,0.724) (S2.1,0.683) (S3.1,0.724) (S4.1,0.6881) (S5,0.709)
    };
    \addlegendentry{$\mathrm{SC}_{\text{macro}}$}
  \end{axis}
\end{tikzpicture}
\caption{Consistency across strategies: schema formatting pass rate and input-aware style uniformity.}
