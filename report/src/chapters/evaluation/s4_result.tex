...
\begin{tikzpicture}
  ...
  \end{axis}
\end{tikzpicture}
\caption{Latency statistics for S4 variants (seconds). The per-field multi-LLM strategy incurs the highest computational cost, with median latencies near 2 minutes and p99 values exceeding 3 minutes. Few-shot prompting (S4.1) slightly reduces extreme latencies compared to S4.0, while the speech-style variant (S4.2) introduces greater variance. Overall, S4 illustrates the trade-off between the robustness gained from per-field consensus and the substantial runtime overhead of coordinating multiple LLM calls.}
