...
The implementation assigns each template field to its own asynchronous extraction task, allowing all fields to be processed in parallel. Each task constructs a field-specific prompt, invokes the LLM, and integrates the result through the system’s normalization and merging logic. Errors in individual fields are isolated so that failures—such as timeouts or malformed responses—affect only the respective field. When a field cannot be extracted reliably, the strategy falls back to a default derived from the previously known value, ensuring robustness without blocking the entire extraction process.
