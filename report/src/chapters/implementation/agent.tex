...
Listing~\ref{lst:ie-api} defines the core input and output schema for the IE agent.\footnote{Supporting types such as \texttt{CurrentFieldValue} and \texttt{FilledField} are given in Appendix~\ref{app:ie-types}.}
...
The verification agent implements \texttt{runVerifier} for single-model outputs and it runs \texttt{runEnsembleVerifier} for multi-model consensus (S3/S4). It acts as a final gate before presenting values to the user or persisting them.

The verification logic operates over \texttt{VerificationInput} and \texttt{VerificationOutput} interfaces, which describe candidate field values, schema information, current values, and verification results. The full TypeScript definitions are provided in Appendix~\ref{app:ver-api}.

Conceptually, the input structure contains either a single map of field identifiers to \texttt{FilledField} objects (for S1/S2) or an array of such maps (for ensemble strategies S3/S4), together with the combined transcript and field schema. The output includes the consolidated \texttt{filled} values, calibrated per-field confidence scores, and a list of detected issues with suggested actions (e.g., re-query, clarification, or manual review). For ensemble strategies, the verifier also records, per field, how the final value was chosen (e.g., selecting GPT, selecting Gemini, merging both, or keeping the current value) together with a short natural-language rationale.

\textbf{Consensus logic (for S3/S4).} For ensemble strategies, the agent uses a judge LLM to evaluate candidate extractions field-by-field. The judge prompt includes candidate values from all models, current field values with lock status, transcript snippets for evidence grounding, schema constraints (type, required flag, allowed options), and decision rules emphasizing transcript consistency, confidence scores, and respect for locks. The judge returns a structured decision per field, choosing between using GPT, using Gemini, merging both, or keeping the current value. For multi-value fields, the \texttt{merge} decision triggers normalization and deduplication before returning the final list.

\textbf{Single-model verification (for S1/S2).} When only one candidate is available, VER performs a series of deterministic checks: completeness (required fields must not be empty), type validation (dates must be parseable, numbers must be finite), cross-field consistency (e.g., ranges and dependencies), and optional grounding checks to verify that extracted values are supported by the transcript. Issues are prioritized according to severity: low-confidence extractions are typically marked for \texttt{"manual\_review"}, while contradictions or severe schema violations suggest \texttt{"clarify"}, prompting a follow-up interaction with the user.
